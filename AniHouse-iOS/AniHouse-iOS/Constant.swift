//
//  Constant.swift
//  AniHouse-iOS
//
//  Created by Jaehoon So on 2022/04/28.
//

import Foundation

class Constant {
    static var forbidWord: [String] = ["18년", "18놈", "개새끼", "씨발", "시팔", "시이팔", "개년", "개돼지", "개새끼", "개쓰레기", "개소리", "개씨발", "개좆", "좆", "개자식", "개지랄", "지랄", "걸레년", "걸레같은", "고자", "깜둥이", "조센징", "쪽바리", "꼴통", "강간", "개가튼년", "개가튼뇬", "개같은년", "개걸레", "개고치", "개너미", "개넘", "개놈", "개년", "개늠", "개똥", "개떵", "개라슥", "개부달", "개부랄", "개새", "개세", "개자식", "귀두", "니기미", "ㅅㅂ", "ㅂㅅ", "ㅅㅍ", "ㄱㅅㄲ", "fuck", "니애미", "니 애미", "니애 미", "니기미", "니애비", "딸딸이", "똥구녁", "띠발뇬", "띠발", "띠펄", "띠풀", "띠풀", "미친넘", "미친놈", "미친년", "미친쉐이", "미틴", "븅신", "섹스", "쎅스", "쉬발", "쉬방", "쉬밸", "쉬이발", "쉬펄", "십쌔", "씹새", "오랄", "bitch", "fuckyou", "nflavor", "nigga", "penis", "pennis", "pussy", "sex", "욕설예시댓글입니다."]
    
    class ImageName {
        static var defaultUserImage: String = "default user image"
        static var defaultImage: String = "default image"
        static var defaultaddImage: String = "add-image"
        static var background: String = "background"
    }
    class ButtonColor {
        static var addButton: String = "add-button"
        static var lightGray: String = "Light Gray"
    }
    class CustomColor {
        static var muchLightGray: String = "much Light Gray"
        static var mainPostCellColor: String = "MainPostCellColor"
        static var muchLightBrown: String = "much light brown"
        static var lightBrown: String = "light brown"
        static var normalBrown: String = "normal brown"
        static var strongBrown: String = "strong brown"
    }
    class MainViewPostModel {
        
    }
}
